...
function sexToysInventoryOnCarryClick(index, category) {
	...
	document.getElementById("stiCarryButton").textContent = toy.carried ? "돌려놓는다" : "가져간다"; // update button text value
...
function sexToysInventoryOnWearClick(index, category) {
	...
	if (setupCategory === "strap-on" && V.worn.under_lower.cursed === 1) {
		Wikifier.wikifyEval('<<trClothes "under_lower" "'+V.worn.under_lower.name+'" "name" "을">>');document.getElementById("stiCursedText").outerHTML =
			`<div id="stiCursedText" class="ssm_fade_in">당신은 ` + T.trResult + ` 벗으려 하지만, 실패한다</div>`;
		return;
	}
	...
	if (setupCategory === "butt_plug") {
		if (V.worn.genitals.cursed === 1 && V.worn.genitals.anal_shield === 1) {
			...
			Wikifier.wikifyEval('<<trClothes "genitals" "'+V.worn.genitals.name+'" "name">>');document.getElementById("stiCursedText").outerHTML =
				`<div id="stiCursedText" class="ssm_fade_in">당신은 ` + T.trResult + `의 애널 보호대 안으로 ` + sextoyPost(toy.name, "name", "을") + ` 밀어넣을 수가 없다</div>`;
			return;
		}
	}
...
function updateCarryCountUI() {
	...
	document.getElementById("carryCount").outerHTML = `<div id="carryCount" class="sti_grid_carried_count">
		소지중인 갯수: <span class="${colour}">${countCarriedSextoys()}/${maxCarried}</span>
	</div>`;
}
...
function setLowerVisibility(desiredVisibility) {
	...
	if (elem !== null) elem.text = !T.lowerVisible ? "하의를 보여준다" : "하의를 숨긴다";
